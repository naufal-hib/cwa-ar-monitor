...
function renderPagination() {
  ...
  if (total <= 1) { elPagination.innerHTML = ''; return; }
    ...
    <div class="flex items-center gap-1 text-sm flex-wrap">
      ...
      ${pages.map(p => p === '...'
        ...
        : `<button onclick="changePage(${p})"
            class="px-3 py-1 rounded border ${p === currentPage ? 'bg-blue-700 text-white border-blue-700' : 'hover:bg-gray-100'}">${p}</button>`
      ).join('')}
  ...
  container.innerHTML = sorted.map((t, i) => `
    <div class="border rounded-lg overflow-hidden mb-2">
      <button onclick="toggleToko('toko-${i}')"
        class="w-full flex items-center justify-between px-4 py-3 bg-white hover:bg-gray-50 text-sm text-left">
        <div class="flex items-center gap-3">
          <span class="font-semibold">${t.nama}</span>
          <span class="text-xs text-gray-500">${t.kode || ''}</span>
          <span class="text-xs text-gray-400">${t.sales || ''}</span>
        </div>
        <div class="flex items-center gap-3">
          <span class="text-xs text-gray-500">${t.invoices.length} invoice</span>
          <span class="font-semibold text-blue-800">${formatRupiah(t.totalSisa)}</span>
        ...
        <table class="w-full text-xs">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="py-2 px-3 text-left">Jatuh Tempo</th>
          ...
          <tbody>
            ${t.invoices.map(inv => {
              ...
              return `<tr class="border-t ${AGING_BG[st.label]}">
                ...
                <td class="py-1.5 px-3">${formatTanggal(inv.tgl_invoice)}</td>
                <td class="py-1.5 px-3">${formatTanggal(inv.tgl_jatuh_tempo)}</td>
...
function renderSummarySales() {
  ...
  tbody.innerHTML = sorted.map(([sales, d]) => `
    <tr class="border-b hover:bg-gray-50 text-sm">
      <td class="py-2 px-4 font-medium">${sales}</td>
      <td class="py-2 px-4 text-center">${d.invoices.toLocaleString('id-ID')}</td>
      <td class="py-2 px-4 text-right">${formatRupiah(d.totalSisa)}</td>
      <td class="py-2 px-4 text-center ${d.badDebt > 0 ? 'text-red-700 font-bold' : ''}">${d.badDebt || '-'}</td>
    </tr>
  `).join('');
}
